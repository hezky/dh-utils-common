...
describe("boolean : method equal", () => {
  ...
  it("equal : true (date)", () => {
    const a1 = [new Date(2016, 1, 1), new Date(2016, 2, 1), new Date(2016, 3, 1), new Date(2016, 4, 1)];
    const a2 = [new Date(2016, 1, 1), new Date(2016, 2, 1), new Date(2016, 3, 1), new Date(2016, 4, 1)];
    assert.ok(utilsArray.equal(a1, a2), "equal array with dates");
  });
  it("equal : true (numbers)", () => {
    const a1 = [1, 2, 3, 4];
    const a2 = [1, 2, 3, 4];
    assert.ok(utilsArray.equal(a1, a2), "equal([1,2,3,4],[1,2,3,4]) === true (numbers)");
  });
  ...
  it("equal : true (strings)", () => {
    const a1 = ["1", "2", "3", "4"];
    const a2 = ["1", "2", "3", "4"];
    assert.ok(utilsArray.equal(a1, a2), "equal(['1','2','3','4'],['1','2','3','4']) === true (strings)");
  });
  it("equal : true (mixed)", () => {
...
describe("array : method is", () => {
  ...
  it("is : false", () => {
    ...
    assert.isFalse(utilsArray.is(new Date("1995-12-17T03:24:00")), "is(new Date('1995-12-17T03:24:00')) === false");
    assert.isFalse(utilsArray.is(() => 1), "is(() => 1) === false");
